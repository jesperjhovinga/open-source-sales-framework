...
description: Turns raw notes from a meeting or call that already happened into a flat, paste-ready CRM CallNote in the BDOwner's BD style. Trigger whenever the user says "summarise my call notes", "CRM note", "log this meeting", the equivalent phrase in the org's own language, or pastes rough notes from a discovery/follow-up meeting and wants them structured. This is the POST-meeting counterpart to cold-call-prep (which preps a call BEFORE it happens) — if the call hasn't happened yet, use cold-call-prep instead. Also trigger after a discovery call when the user wants the account logged before drafting a follow-up.
---
...
5. **The org angle** — where the org plausibly helps, in the BDOwner's framing, and explicitly where it does *not* (the honest boundary is part of the credibility — see tone of voice).
...
```
ACCOUNT: [Company] — [context tag, e.g. "second follow-up call"]
...
- Match the language of the notes — write the CallNote in whatever language the notes came in.
