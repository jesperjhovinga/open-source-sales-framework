# CallNote template (annotated) + worked example

Read this when you need the full structure or a concrete model to match. The SKILL.md body has the short version; this is the reference.

## Annotated template

```
ACCOUNT: [Company] — [short context tag]

Account overview (rough): [One paragraph. What the company is, size, market, age,
ownership, core product, any platform/strategy shift in motion. EVERY number that
didn't come from the person in the room is flagged: "~145 employees", "est. revenue
~[currency]22M (third-party, rough)". A platform transition or M&A move is usually
the most useful hook — call it out.]

Entry point: [The single specific reason there's a conversation. A felt pain, a
trigger event, a mutual connection, a case of ours that resonated. Be concrete:
"urgent on improving service around their turnkey solution; pain lives in their
customer-service team" beats "interested in working together".]

Read from the meeting: [Your honest assessment. How warm was the attitude? Is the
fit real or only on paper? Where could it go? Mark inference explicitly:
"Realistic read: likely to go cold without us driving it." If the org fumbled
something in the room, log it so it gets fixed before re-engaging.]

Opportunity / Our angle: [Where the org plausibly helps — in the BDOwner's framing. AND
the honest boundary: what the org deliberately does not take on. The boundary is not a
weakness here; per the tone of voice it earns trust on the rest.]

[Optional lines — include ONLY if the notes contain them:]
Business model: [...]
Competitive landscape: [...]
Ecosystem fit: [...]

Next actions: 1) [...] 2) [...] 3) [...]  (concrete, owner where known)

Open questions: [What's still unknown. Do not fabricate answers. If scope/fit is
unproven, say so.]
```

## Worked example (English)

```
ACCOUNT: [Account] — follow-up

Account overview (rough): [Product category] company, founded [year], HQ [city]
([country]), founded by domain scientists. ~25 employees across Europe/Asia/North
America (small org), est. revenue ~[currency]25M (third-party, rough). Recently opened a
second office in [location]. Core stack: proprietary hardware + [control software]
+ sensor tech ([sensor product], full release 2026). Strong on sensor and data;
differentiation is data-driven control.

Ecosystem fit: Part of [network], same as us and [partner] — warm network
context, not cold.

Read from the meeting: Open and promising. Genuinely strong on the underlying
science, but light on software-product muscle.

Entry point / opportunity: They need a robust digital product wrapped around their
services — specifically around [sensor product]. Great data, but it needs
productizing into something dependable and customer-facing. That's squarely
ours to build.

Key signal: IT team is small (<5, from our conversations). Likely: for a company
needing to productize in-house tech it can't build alone, that's a good sign, not
a caution flag — a strong candidate to land a real project, not just a conversation.

Next actions: 1) Prep the digital-product narrative around [sensor product] for
22 June. 2) Confirm logistics + who attends (esp. whoever owns IT/product).
3) Sketch a rough productization concept to react to. 4) Define discovery + scope
questions.

Open questions: Opportunity scope and timeline unproven — qualify at the meeting.
Who internally owns the product roadmap?
```

## Note on the over-claim trap (real case)

From a real prospect session: notes said "second-line for [Account B]" and, separately, "first/second-line partly via a sister company in India." Merging these into "second-line for [Account B] via India" invented a link neither note supported. The correct CallNote keeps them as two facts. When in doubt, under-commit and ask the BDOwner.
